...
def home(code="INFO1601"):
  enrolled = [
      student for student in Student.query.all() if student.is_enrolled(code)
  ]
  not_enrolled = [
      student for student in Student.query.all()
      if not student.is_enrolled(code)
  ]
...
@app.route('/enroll/<id>/<code>', methods=['GET'])
@jwt_required()
def enroll(id, code):
...
@app.route('/unenroll/<id>/<code>', methods=['GET'])
@jwt_required()
def unenroll(id, code):
  student = Student.query.filter_by(id=id).first()
  if student:
    student.unenroll(code)
  return redirect(request.referrer)
